...
- **CMS**: PayloadCMS 3.0 (Next.js Native)
...
async function getHerb(slug: string): Promise<Herb | null> {
  const payload = await getPayload({ config })
...
export default async function HerbPage({
  ...
  return (
    <article>
      <h1>{herb.name}</h1>
      <p className="italic text-muted-foreground">{herb.scientificName}</p>
      
      {herb.tcmProperties && (
        <div className="mt-4 flex gap-2">
          <span className="badge">{herb.tcmProperties.temperature}</span>
          {herb.tcmProperties.taste?.map(taste => (
            <span key={taste} className="badge">{taste}</span>
          ))}
        </div>
      )}
...
export default async function DashboardPage() {
  ...
  return (
    <div>
      <h1>Welcome, {payloadUser.docs[0]?.name}</h1>
      <p>Role: {payloadUser.docs[0]?.role}</p>
    </div>
  )
}
```
...
async function migrateData() {
  ...
  for (const strapiHerb of strapiHerbs.data) {
    try {
      await payload.create({
        collection: 'herbs',
        data: {
...
- [Payload + Next.js Guide](https://payloadcms.com/blog/payload-30-the-first-cms-that-installs-directly-into-any-nextjs-app)
- [Migration Guide v2 → v3](https://payloadcms.com/docs/migration-guide)
- [Verscienta Health Docs](https://github.com/The-Verscienta/Verscienta-Health/tree/main/docs)
