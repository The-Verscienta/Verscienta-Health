/**
 * Trefle API Health Check Endpoint
 *
 * Public health check endpoint for monitoring Trefle API status.
  ...
  try {
    ...
    if (!trefleClientEnhanced.isConfigured()) {
      return NextResponse.json(
        {
          status: 'unconfigured',
          message: 'Trefle API is not configured',
          score: 0,
          circuitState: 'OPEN',
    ...
    console.error('[Trefle Health API] Error checking health:', error)
    return NextResponse.json(
      {
        status: 'error',
        message: 'Failed to check Trefle API health',
        error: error.message,
        timestamp: new Date().toISOString(),
      },
      { status: 500 }
