...
const nextConfig: NextConfig = {
  ...
  images: {
    ...
    // Disable image optimization to avoid Sharp errors with Payload UI assets
    unoptimized: true,
  },
  ...
  async redirects() {
    return [
      // Removed /admin redirect - Payload CMS is now integrated into Next.js app
    ]
  },
...
// Wrap with Payload, next-intl, PWA plugin, Bundle Analyzer
// Note: Sentry temporarily disabled in production due to standalone build issues
const configWithPlugins = withPayload(
  withNextIntl(
    bundleAnalyzer(
      withPWA({
        dest: 'public',
