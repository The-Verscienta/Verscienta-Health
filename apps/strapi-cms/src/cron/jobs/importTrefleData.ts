...
function isHerbCandidate(plant: any): boolean {
  ...
  if (plant.specifications?.growth_habit && goodHabits.includes(plant.specifications.growth_habit.toLowerCase())) {
    return true
  }

  // Exclude trees
  if (plant.specifications?.growth_habit && plant.specifications.growth_habit.toLowerCase() === 'tree') {
    return false
  }
